...
void opcontrol() {
  ...
  while (true) {
    ...
    //lift.set_active_brake(pros::E_MOTOR_BRAKE_HOLD);
    ...
    if (controller2.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_B)) {
  doinker.set(!doinker.get());
} 

 if (controller2.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_X)) {
  doi.set(!doi.get());
} 
