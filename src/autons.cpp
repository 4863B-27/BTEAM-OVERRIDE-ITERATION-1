...
void Right_Blue() {//for some reason id isn't calling back prolly internet i know it sounds stuipid but id wasn't rejected at school 
 ...
 /* lift.move(-100);
  ...
  chassis.pid_drive_set(5.5_in, 80, true);//50//10in//5in
  chassis.pid_wait();
...
//not here before bottom two commands
 ...
 // right or left side for auton is shown by facing the opposit color diagnal from the match load and right of you is right side etc
  ...
  chassis.pid_turn_relative_set(27, 70); // turn to face standing pin//35degrees//28deg
  chassis.pid_wait();
  ...
  chassis.pid_drive_set(8_in, 110);//not measured dont change without lucas diaz 
  chassis.pid_wait();

  chassis.pid_drive_set(-3_in, 110);//measured 
  chassis.pid_wait();

  chassis.pid_turn_set(45,90);//if turns wrong way change to negative
  chassis.pid_wait();

  chassis.pid_drive_set(10_in, 110);//not measured dont change without lucas diaz 
  chassis.pid_wait();

  chassis.pid_drive_set(-4_in, 110);//not measured dont change without lucas diaz 
  chassis.pid_wait();

  chassis.pid_turn_set(-35,90);//if turns wrong way change to positive
  chassis.pid_wait();
  lift.move(45);
  //set_lift_position(45,100);
  chassis.pid_drive_set(-4_in, 110);//not measured dont change without lucas diaz 
